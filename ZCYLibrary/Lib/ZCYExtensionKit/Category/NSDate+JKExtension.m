...
+ (NSString *)jk_formatYMD:(NSDate *)date {
    return [NSString stringWithFormat:@"%lu-%02lu-%02lu",(unsigned long)[date jk_year],(unsigned long)[date jk_month], (unsigned long)[date jk_day]];
}
...
//long long -> datestring:
+(NSString*)dateFromLongLong:(double)msSince1970{
    
    NSTimeInterval interval = msSince1970;
    NSDate *online = [NSDate date];
    online = [NSDate dateWithTimeIntervalSince1970:interval];
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyy.MM.dd"];
    NSString *showtimeNew = [formatter stringFromDate:online];
    return showtimeNew;
    
}

//long long -> datestring:
+(NSString*)dateYYMMDDHHMMFromLongLong:(double)msSince1970{
