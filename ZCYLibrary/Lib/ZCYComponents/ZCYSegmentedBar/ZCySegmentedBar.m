...
#import "ZCYSegmentedBar.h"

#define MainScreen_W [UIScreen mainScreen].bounds.size.width

@interface ZCYSegmentedBar () <UIScrollViewDelegate>

@end

@implementation ZCYSegmentedBar

- (instancetype)initWithFrame:(CGRect)frame titleArray:(NSArray *)titleArray subviewArray:(NSArray *)subviewArray {
    if (self = [super initWithFrame:frame]) {
        _bottomScrollView=[[UIScrollView alloc] initWithFrame:CGRectMake(0, _ZCYSegmentedView.frame.size.height, MainScreen_W, self.bounds.size.height-_ZCYSegmentedView.bounds.size.height)];
        [self addSubview:_bottomScrollView];
        _bottomScrollView.contentSize=CGSizeMake(MainScreen_W*subviewArray.count, self.bounds.size.height-_ZCYSegmentedView.bounds.size.height);
        ...
        //Click ZCYSegmentedBar Event
        _ZCYSegmentedView=[[ZCYSegmentedView alloc] initWithFrame:CGRectMake(0, 0, MainScreen_W, 35) titles:titleArray clickBlick:^void(NSInteger index) {
            [_bottomScrollView setContentOffset:CGPointMake(MainScreen_W * (index-1), 0)];
            [self.ZCYDelegate segmentedBar:self didSelectAtIndex:(index-1)];
        }];
        [self addSubview:_ZCYSegmentedView];
        
        //scroll ZCYSegmentedBar Event
        for (int i = 0; i < subviewArray.count; i++) {
            UIView *contentView = (UIView *)subviewArray[i];
            contentView.frame=CGRectMake(MainScreen_W * i, _ZCYSegmentedView.bounds.size.height, MainScreen_W, _bottomScrollView.frame.size.height-_ZCYSegmentedView.bounds.size.height);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    [_ZCYSegmentedView updateselectLineFrameWithoffset:point.x];
}

-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView == _bottomScrollView) {
        NSInteger index = _bottomScrollView.contentOffset.x/MainScreen_W;
        _ZCYSegmentedView.defaultIndex = index+1;
        [self.ZCYDelegate segmentedBar:self didSelectAtIndex:index];
    }
...
@implementation ZCYSegmentedView
...
@implementation CALayer (ZCYSegmentedBar)
...
@implementation NSString (ZCYSegmentedBar)
/**
 *  @brief 计算文字的高度
 *
