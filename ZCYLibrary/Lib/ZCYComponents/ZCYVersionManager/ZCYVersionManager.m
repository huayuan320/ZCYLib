...
#import "ZCYVersionManager.h"

#import "MBProgressHUD.h"

@implementation ZCYVersionManager
...
+ (void)ZCY_updateVersionWithAppStoreID:(NSString *)appStoreID
    ...
    //确定请求路径
    NSURL *ZCYURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://itunes.apple.com/cn/lookup?id=%@", appStoreID]];
    
    //创建请求对象 请求对象内部默认已经包含了请求头和请求方法（GET）
    NSURLRequest *ZCYURLRequest = [NSURLRequest requestWithURL:ZCYURL];
    ...
    */
    NSURLSession *ZCYURLSession = [NSURLSession sessionWithConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]
                                                                   delegate:nil
                                                              delegateQueue:[NSOperationQueue mainQueue]];
    
    //根据会话对象创建一个Task(发送请求）
    NSURLSessionDataTask *ZCYURLSessionDataTask = [ZCYURLSession dataTaskWithRequest:ZCYURLRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if (data == nil) {
            NSLog(@"您没有连接网络");
            [ZCYVersionManager ZCY_showVersionMessage:@"您没有连接网络 !"];
            return;
        }
        ...
        if (resultsArray.count < 1) {
            ...
            [ZCYVersionManager ZCY_showVersionMessage:@"此APPID为未上架的APP或者查询不到 !"];
            return;
        }
        ...
        BOOL isNewVersion = [ZCYVersionManager ZCY_compareVersionsFormAppStoreVersion:appStoreVersion currentAppVersion:currentVersion];
        ...
        if(isNewVersion == YES) {
            ...
            NSLog(@"您当前版本已经最新");
            [ZCYVersionManager ZCY_showVersionMessage:@"您当前版本已经最新 !"];
        }
    }];
    
    //执行任务
    [ZCYURLSessionDataTask resume];
}

+ (void)ZCY_showVersionMessage:(NSString *)versionMessage {
...
+ (BOOL)ZCY_compareVersionsFormAppStoreVersion:(NSString*)AppStoreVersion currentAppVersion:(NSString*)currentAppVersion {
